...
class ClusteringTimelineGenerator():
    def __init__(self,
        ...
        self.ex_sum = summarizers.CentroidOpt()
    ...
    def predict(self,
        ...
        if vectorizer is None:
            ...
            clusters = self.clusterer.cluster(collection, vectorizer)
        ...
        for c in ranked_clusters:
            ...
            date = c.time.date()
            if date < collection.start.date() or date > collection.end.date():
                print(f"start {collection.start.date()} end {collection.end.date()} skip {date}")
                continue
            #if str(date) in ['2011-03-02', '2011-03-19', '2011-04-04', '2011-04-07', '2011-04-20']:
            #    print(date)
            #    c_sents = self._select_sents_from_cluster(c)
            #    sents = [s.raw for s in c_sents]
            #    print(sents)
            if self.unique_dates and date in date_to_summary:
                continue
            if collection.name =='libya':
                if str(date) == '2011-02-21':
                    continue
            c_sents = self._select_sents_from_cluster(c)
    ...
    def rl(self,
        ...
        for c in ranked_clusters:
            ...
            if self.unique_dates and date in date_to_summary:
                continue
            c_sents = self._select_sents_from_cluster(c)
            summary, explained_vars = self.summarizer.rl(args, c_sents, env, k=max_summary_sents, filter=sent_filter)
            cluster_explained_vars.append(explained_vars)
...
class SBERTClusterer(Clusterer):
    def cluster(self, collection, vectorizer):
        ...
        print(f'times: {len(set(times))} articles: {len(articles)} '
              f'clusters: {clustering_model.n_clusters_}')

        clusters = []
        for c in range(clustering_model.n_clusters_):
            c_vectors = [X[t] for t, ct in enumerate(cluster_assignment) if ct==c]
            c_articles = [articles[t] for t, ct in enumerate(cluster_assignment) if ct==c]
            centroid = np.mean(np.vstack(c_vectors), axis=0)
            cluster = Cluster(c_articles, c_vectors, centroid=centroid)
            clusters.append(cluster)

        return clusters

class SBERTAPClusterer(Clusterer):
    def cluster(self, collection, vectorizer):
        articles = list(collection.articles())
        texts = ['{} {}'.format(a.title, a.text) for a in articles]
        X = vectorizer.encode(texts)
        ...
        print(f'times: {len(set(times))} articles: {len(articles)} '
              f'clusters: {len(clustering_model.cluster_centers_indices_)}')
        center_id = clustering_model.cluster_centers_indices_

        clusters = []
        for c in clustering_model.cluster_centers_indices_:
            c_vectors = [X[t] for t, ct in enumerate(cluster_assignment) if center_id[ct]==c]
            c_articles = [articles[t] for t, ct in enumerate(cluster_assignment) if center_id[ct]==c]
            centroid = np.mean(np.vstack(c_vectors), axis=0)
            cluster = Cluster(c_articles, c_vectors, centroid=centroid)
            clusters.append(cluster)
...
class SBERTMarkovClusterer(Clusterer):
    def __init__(self, max_days=1):
        self.max_days = max_days

    def cluster(self, collection, vectorizer) -> List[Cluster]:
        print("SBERTMarkovClustering...")
        articles = list(collection.articles())
        texts = ['{} {}'.format(a.title, a.text) for a in articles]
        X = vectorizer.encode(texts)
        times = [a.time for a in articles]
        ...
        for c in idx_clusters:
            ...
            Xc = sparse.vstack(sparse.csr_matrix(c_vectors))
    ...
    def temporal_graph(self, X, times):
        ...
        for n in range(total_days + 1):
            ...
            if n == 0 or len(window) == 1:
                ...
                if len(indices) == 0:
                    continue
                X_n = sparse.vstack([sparse.csr_matrix(X[i]) for i in indices])
                ...
                X_prev = sparse.vstack([sparse.csr_matrix(X[i]) for i in prev_indices])
                X_new = sparse.vstack([sparse.csr_matrix(X[i]) for i in new_indices])
...
class TemporalMarkovClusterer(Clusterer):
    ...
    def cluster(self, collection, vectorizer) -> List[Cluster]:
        print("TemporalMarkovClustering...")
        articles = list(collection.articles())
        texts = ['{} {}'.format(a.title, a.text) for a in articles]
        try:
            X = vectorizer.transform(texts)
